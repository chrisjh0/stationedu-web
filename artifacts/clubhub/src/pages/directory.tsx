...
import { useEnrollInClub } from "@workspace/api-client-react";
...
export default function DirectoryPage() {
  ...
  }, [allClubs, search, typeFilter, dayFilter, statusFilter]);
    ...
    enrollMutation.mutate(
      { id: clubId },
      {
        onSuccess: () => {
          ...
          setAllClubs((prev) =>
            prev.map((c) => (c.id === clubId ? { ...c, is_enrolled: true } : c))
          );
        },
        onError: (err) => {
            ...
            {filteredClubs.map((club) => (
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  <div
                    className={`w-14 h-14 rounded-2xl flex items-center justify-center text-white font-bold text-2xl ${getClubColor(club.id)}`}
                  >
                    {club.profile_photo ? (
                      <img
                  ...
                  <div className="flex flex-col gap-1 items-end">
                    <span className="bg-surface-container text-secondary text-xs px-2 py-1 rounded-md font-medium">
                      {club.type}
                    </span>
