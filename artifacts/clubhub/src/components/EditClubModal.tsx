...
function EditClubForm({ club, onClose }: { club: Club; onClose: () => void }) {
  ...
  const [leaders, setLeaders] = useState(
    club.leaders?.length ? club.leaders : [{ name: "", role: "", email: "" }]
  );
              ...
              <div key={i} className="flex gap-2 items-start bg-surface p-3 rounded-xl border border-outline-variant/30">
                <div className="grid grid-cols-3 gap-2 flex-grow">
                  <Input value={leader.name} onChange={e => {
                    const newL = [...leaders]; newL[i] = { ...newL[i], name: e.target.value }; setLeaders(newL);
                  }} placeholder="Name" className="h-9 text-sm" />
                  <Input value={leader.role} onChange={e => {
                    const newL = [...leaders]; newL[i] = { ...newL[i], role: e.target.value }; setLeaders(newL);
                  }} placeholder="Role" className="h-9 text-sm" />
                  <Input value={leader.email} onChange={e => {
                    const newL = [...leaders]; newL[i] = { ...newL[i], email: e.target.value }; setLeaders(newL);
